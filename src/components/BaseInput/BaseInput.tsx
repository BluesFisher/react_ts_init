...
const BaseInput: React.FC<IProps> = (propsIn: IProps) => {
    ...
    const onFocus = () => {
        ...
        if (!iconDom) {
            iconDom = document.getElementsByClassName(iconClassName)[0] as HTMLElement;
        }
    ...
    const renderInput = (): JSX.Element => {
        ...
        if (typeof value === 'string') {
            return (
                <span className={`field-container ${userClass}`}>
                    {label && <label className="field-label">{label}</label>}
                    {props.children}
                    <input className="field-input" type={type} name={name} value={value} placeholder={placeholder} onChange={handleInputChange} onFocus={onFocus} onBlur={onBlur} />
                </span>
            );
        }
    ...
    const renderCheckbox = (): JSX.Element => {
        ...
        if (typeof value === 'boolean') {
            return (
                <label className={`field-checkbox ${userClass}`}>
