...
const PhotoModal: React.FC<IProp> | null = (propsIn: IProp) => {
    const props: IProp = { ...JSON.parse(JSON.stringify(defaultProps)), ...propsIn };
    const [picIndex, setPicIndex] = useState(0);
    const [picArrowOpacity, setPicArrowOpacity] = useState(0);

    const onPicPopCancel = () => {
        if (props.onPicPopCancel) {
            props.onPicPopCancel();
        }
    };
            ...
            <div className="pic-pop-container">
                ...
                <div className="pic-container" onMouseEnter={onPicMouseEnter} onMouseLeave={onPicMouseLeave}>
                    ...
                    <div className="pic-move-ctrl" style={{ marginLeft: `${-picIndex * MARGIN_LEFT}rem` }}>
                        {props.picInfo.length <= 0
                            ? '暂无数据'
                            : props.picInfo.map((item: any) => (
                                  <div className="pic-content" key={item}>
                                      <img src={item} alt="" />
                                  </div>
                              ))}
                    </div>
