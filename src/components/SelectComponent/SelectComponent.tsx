...
const SelecComponent: React.FC<InterPropsType> = (propsIn: InterPropsType) => {
            ...
            <div className="search-select search-common">
                ...
                <MySelect mode={mode} onChange={handleSelect} showArrow={showArrow} value={value} disabled={disabled} defaultValue={defaultValue}>
                    {chooseData.map((item: InterDataType) => {
                        ...
                        return (
                            <MyOption value={name} key={code} title={name}>
                                {name}
                            </MyOption>
                        );
                    })}
                ...
                <div className="select-valuebox">
                    {value.length > 0 ? <div className="select-value">{value.join(',')}</div> : defalutlist.length > 0 ? '' : <div className="select-place">请选择</div>}
                </div>
                <div className="select-valuebox">{defalutlist.length > 0 ? <div className="select-value">{defalutlist.join(',')}</div> : <div className="select-place" />}</div>
            </div>
        );
    };
